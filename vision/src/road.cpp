...
cv::Mat worldcoordinate(cv::Mat img) {
  ...
  for (uint16_t i = 0; i < world_y; i++) {
    ...
    for (uint16_t j = 0; j < world_x; j++) {
      float xx =
          xsize / 2 +
          Kx * (j - (world_x / 2)) / ((world_y - i) * cos(angle * M_PI / 180) +
                                      heigh * sin(angle * M_PI / 180));
      float yy = ysize / 2 -
                 Ky * ((world_y - i) * sin(angle * M_PI / 180) -
                       heigh * cos(angle * M_PI / 180)) /
                     ((world_y - i) * cos(angle * M_PI / 180) -
                      heigh * sin(angle * M_PI / 180));
      if (xx > 0 && xx < xsize && yy > 0 && yy < ysize) {

        if (img.at<Vec3b>(yy, xx)[0] == 255 && img.at<Vec3b>(yy, xx)[1] == 0 &&
            img.at<Vec3b>(yy, xx)[2] == 0) {
          ipm_data[j * 3 + 0] = 255;
          ipm_data[j * 3 + 1] = 255;
          ipm_data[j * 3 + 2] = 255;
        } else {
  ...
  Mat cat_roi(ipm_img,
              cv::Rect(0, y_start - 131, world_x - 1, world_y - y_start + 131));
  drawLines(cat_roi); //將畫面直線延伸

  Mat imgroi =
      ipm_img(cv::Rect(0, y_start - 131, world_x - 1, world_y - y_start));
...
cv::Mat dis_dot(cv::Mat img) {
  ...
  line(img, Point(img.cols / 2, y_start),
       Point(img.cols / 2 -
                 50 * cos(CV_PI / 2 + (car_vec - 100) * 0.02 * (CV_PI / 6)),
             y_start - 50 * sin(CV_PI / 2) +
                 (car_vec - 100) * 0.02 * (CV_PI / 6)),
       Scalar(255, 0, 0), 5);
...
void ParmIsChangeCallback(const std_msgs::Bool::ConstPtr &msg) {
  ...
  if (msg->data) {
    ...
    system("rosparam dump ~/linux/catkin_ws/src/a_launch/config/Parameter.yaml "
           "/golf");
  }
}
void Parameter_getting(bool file) {
  ros::NodeHandle n;
  if (file) {
    system("rosparam load ~/linux/catkin_ws/src/a_launch/config/Parameter.yaml "
           "/golf");
    n.getParam("/golf/high", heigh);
    n.getParam("/golf/angle", angle);
    n.getParam("/golf/Kx", Kx);
...
void drawLinesP(Mat &input, Mat &deaw_pic) {
  ...
  for (int i = 0; i < lines.size(); i++) {
    line(deaw_pic, Point(lines[i][0], lines[i][1]),
         Point(lines[i][2], lines[i][3]), Scalar(255, 0, 0), 3);
  }
  lines.clear();
}
void drawLines(Mat &input) {
  ...
  for (int i = 0; i < liness.size(); i++) {
    // line(input, Point(liness[i][0], liness[i][1]), Point(liness[i][2],
    // liness[i][3]), Scalar(255, 0, 0), 3);

    int m = (liness[i][2] - liness[i][0] != 0)
                ? (liness[i][3] - liness[i][1]) / (liness[i][2] - liness[i][0])
                : 99999;
    ...
    line(
        input, Point(linedott[0].x, linedott[0].y),
        Point(linedott[linedott.size() - 1].x, linedott[linedott.size() - 1].y),
...
int main(int argc, char *argv[]) {
  ...
  Parameter_getting(1);
  ros::Subscriber sub_parm =
      n.subscribe("/ParmIsChange", 10, ParmIsChangeCallback);
  ros::Subscriber sub_car_wheel = n.subscribe("/car_wheel", 10, carwheel_vec);
  image_transport::ImageTransport it(n);
  image_transport::Subscriber sub_image =
      it.subscribe("usb_cam/image_raw", 1, imageCallback, ros::VoidPtr(),
  ...
  ros::Publisher dis_pub =
      n.advertise<std_msgs::Float64MultiArray>("/roadDis", 1);
  ...
  while (ros::ok()) {
    ...
    Mat1b mask, mask2;
    inRange(hsv, Scalar(HSV[1], HSV[3], HSV[5]), Scalar(HSV[0], HSV[2], HSV[4]),
            mask);
    ...
    // msg_image = cv_bridge::CvImage(std_msgs::Header(), "bgr8",
    // img).toImageMsg();
    // pub_image.publish(msg_image);
    //////
    Mat src, src_gray;
    src = Main_frame.clone();
    ...
    HoughCircles(src_gray, circles, CV_HOUGH_GRADIENT, 1, src_gray.rows / 20,
                 200, 100, 0, 0);
