...
pcl::PointCloud<pcl::PointXYZ>::Ptr
    back_ground(new pcl::PointCloud<pcl::PointXYZ>);
pcl::PointCloud<pcl::PointXYZ>::Ptr cloud(new pcl::PointCloud<pcl::PointXYZ>);
pcl::PointCloud<pcl::PointXYZ>::Ptr
    id_online(new pcl::PointCloud<pcl::PointXYZ>);
pcl::PointCloud<pcl::PointXYZ>::Ptr
    Trajectory_point_msg(new pcl::PointCloud<pcl::PointXYZ>);
...
pcl::ConditionAnd<pcl::PointXYZ>::Ptr
    range_cond(new pcl::ConditionAnd<pcl::PointXYZ>); //濾除 roi以外的點雲
...
void two_point_filter(const VPointCloud::ConstPtr &scan,
                      const VPointCloud::ConstPtr &inputpoint, unsigned npoints,
                      unsigned nin_points, size_t &obs_count,
                      size_t &empty_count) {
  int grid_dim_ = 120;
  float m_per_cell_ = 0.1;
  float height_diff_threshold_ = 0.5;
  ...
  for (int x = 1; x < grid_dim_ - 1; x++) {
    for (int y = 1; y < grid_dim_ - 1; y++) {
      if (num_obs[x][y] > 0 && num_obs[x - 1][y] == 0 &&
          num_obs[x + 1][y] == 0 && num_obs[x][y - 1] == 0 &&
          num_obs[x][y + 1] == 0)
        num_obs[x][y] = 0;
    }
  }
...
Eigen::Matrix4f CreateRoatationMatrix(Vector3f angle_before,
                                      Vector3f angle_after) {
...
bool customCondition(const pcl::PointXYZ &seedPoint,
                     const pcl::PointXYZ &candidatePoint,
                     float squaredDistance) {
  // Do whatever you want here.做你想做的条件的筛选
  if (candidatePoint.y <
      seedPoint
          .y) //如果候选点的Y的值小于种子点的Y值（就是之前被选择为聚类的点），则不满足条件，返回假
    return false;
...
void pointCallback(const VPointCloud::ConstPtr &msg) {
  ...
  //     // apply filter
  //     outrem.filter (*cloud_filtered);     //执行条件滤波   在半径为0.8
  ...
  //  viewer.showCloud(cloud_filtered);
  pcl::PointCloud<pcl::PointXYZ>::Ptr point_mis(
      new pcl::PointCloud<pcl::PointXYZ>);
  ////////////////////////////////////////和背景影像剪
  for (int i = 0; i < cloud_filtered->points.size(); i++) {
    int K = 1; //设搜索一个点，即：K为1.
    std::vector<int> pointIdxNKNSearch(
        K); //设置一个类似数组的东西，（我对于C++中vector的理解直接把它当成了数组），pointIdxNKNSearch（K）的长度为K，用于存放点索引
    std::vector<float> pointNKNSquaredDistance(
        K); //同样一个长度为K的数组pointNKNSquaredDistance，用于存放距离的平方

    if (kdtree.nearestKSearch(cloud_filtered->points[i], K, pointIdxNKNSearch,
                              pointNKNSquaredDistance) >
        0) //前面定义了kdtree作为搜索对象，然后就可以用它来调用系统的nearestKSearch函数了，注意后面的参数，分别是上面已经定义好的。
    {
  ...
  int j = 0;
  pcl::PointCloud<pcl::PointXYZI>::Ptr point_show(
  ...
  for (std::vector<pcl::PointIndices>::const_iterator it =
    ...
    for (std::vector<int>::const_iterator pit = it->indices.begin();
      ...
      point_color.x = cloud_filtered->points[*pit].x;
      point_color.y = cloud_filtered->points[*pit].y;
      point_color.z = cloud_filtered->points[*pit].z;
  ...
  if (count_img == 3) {
    ...
    if (id_online->points.size() && obj_cloud->points.size()) {
      for (int i = 0; i < id_online->points.size(); i++) {
        ...
        if (tree_obj.nearestKSearch(id_online->points[i], K, pointIdxNKNSearch,
                                    pointNKNSquaredDistance) >
            0) //前面定义了kdtree作为搜索对象，然后就可以用它来调用系统的nearestKSearch函数了，注意后面的参数，分别是上面已经定义好的。
        {
          if (pointNKNSquaredDistance[0] < 1.5) { // r距離大於1公分
            ...
            double th = atan((id_online->points[i].y - pose_temp.position.y) /
                             (id_online->points[i].x - pose_temp.position.x));
            if ((id_online->points[i].y - pose_temp.position.y) > 0 &&
                    (id_online->points[i].x - pose_temp.position.x) > 0 ||
                (id_online->points[i].y - pose_temp.position.y) < 0 &&
                    (id_online->points[i].x - pose_temp.position.x) > 0) {
              th -= M_PI;
            }
...
int main(int argc, char *argv[]) {
  ...
  ros::Subscriber velodyne_scan_ =
      n.subscribe("/velodyne_points", 10, pointCallback,
                  ros::TransportHints().tcpNoDelay(true));
  int count = 0;

  // pcl::FieldComparison<pcl::PointXYZ>::ConstPtr cond_y1(new
  // pcl::FieldComparison<pcl::PointXYZ>("y", pcl::ComparisonOps::GT, -5.0f));
  // range_cond->addComparison(cond_y1);
  // pcl::FieldComparison<pcl::PointXYZ>::ConstPtr cond_y2(new
  // pcl::FieldComparison<pcl::PointXYZ>("y", pcl::ComparisonOps::LT, 5.0f));
  // range_cond->addComparison(cond_y2);
  // pcl::FieldComparison<pcl::PointXYZ>::ConstPtr cond_x1(new
  // pcl::FieldComparison<pcl::PointXYZ>("x", pcl::ComparisonOps::GT, -5.0f));
  // range_cond->addComparison(cond_x1);
  // pcl::FieldComparison<pcl::PointXYZ>::ConstPtr cond_x2(new
  // pcl::FieldComparison<pcl::PointXYZ>("x", pcl::ComparisonOps::LT, 5.0f));
  // range_cond->addComparison(cond_x2);

  pcl::FieldComparison<pcl::PointXYZ>::ConstPtr cond_z1(
      new pcl::FieldComparison<pcl::PointXYZ>("z", pcl::ComparisonOps::GT,
                                              -1.2f));
  range_cond->addComparison(cond_z1);
  pcl::FieldComparison<pcl::PointXYZ>::ConstPtr cond_z2(
...
// 	{
// 		double distant = Math::Sqrt(Math::Pow((P1.x - P2.x), 2) +
...
// 				{
// 					// unite both trees
// 					int rank = nodes[root][RANK], rank2 =
// nodes[root2][RANK];
// 					if (rank > rank2)
...
//  pcl::PointCloud<pcl::PointXYZ>::Ptr transform_point(new
//  pcl::PointCloud<pcl::PointXYZ>);
...
// //     //LE less than
// //     pcl::FieldComparison<pcl::PointXYZ>::ConstPtr cond_z1(new
// pcl::FieldComparison<pcl::PointXYZ>("z", pcl::ComparisonOps::GT, -5.0f));
// //     range_cond->addComparison(cond_z1);
// //     pcl::FieldComparison<pcl::PointXYZ>::ConstPtr cond_z2(new
// pcl::FieldComparison<pcl::PointXYZ>("z", pcl::ComparisonOps::LT, -3.0f));
// //     range_cond->addComparison(cond_z2);
// //     pcl::FieldComparison<pcl::PointXYZ>::ConstPtr cond_y1(new
// pcl::FieldComparison<pcl::PointXYZ>("y", pcl::ComparisonOps::GT, 3.0f));
// //     range_cond->addComparison(cond_y1);
// //     pcl::FieldComparison<pcl::PointXYZ>::ConstPtr cond_y2(new
// pcl::FieldComparison<pcl::PointXYZ>("y", pcl::ComparisonOps::LT, 6.0f));
// //     range_cond->addComparison(cond_y2);
// //     pcl::FieldComparison<pcl::PointXYZ>::ConstPtr cond_x1(new
// pcl::FieldComparison<pcl::PointXYZ>("x", pcl::ComparisonOps::GT, 0.0f));
// //     range_cond->addComparison(cond_x1);
// //     pcl::FieldComparison<pcl::PointXYZ>::ConstPtr cond_x2(new
// pcl::FieldComparison<pcl::PointXYZ>("x", pcl::ComparisonOps::LT, 20.0f));
// //     range_cond->addComparison(cond_x2);
// //     //创建滤波器并用条件定义对象初始化
...
// //     pcl::ModelCoefficients::Ptr plane_coefficients ( new
...
// //////////////////////////////////////////////////////////////////////////
// // pcl::transformPointCloud(*msg, *transform_point, CreateRoatationMatrix({0,
// 0, 0},{0.0f,0.0f,1.0f}));
//                                                                          //
//                                                                          x
//                                                                          y
...
//  pcl::PointCloud<pcl::PointXYZ>::Ptr pointCloud_filter(new
//  pcl::PointCloud<pcl::PointXYZ>);
// /////////濾掉地面訊號
//     pcl::ConditionAnd<pcl::PointXYZ>::Ptr range_cond(new
//     pcl::ConditionAnd<pcl::PointXYZ>);
//     pcl::FieldComparison<pcl::PointXYZ>::ConstPtr cond_z1(new
//     pcl::FieldComparison<pcl::PointXYZ>("z", pcl::ComparisonOps::GT, 3.3f));
//     range_cond->addComparison(cond_z1);
//     pcl::FieldComparison<pcl::PointXYZ>::ConstPtr cond_z2(new
//     pcl::FieldComparison<pcl::PointXYZ>("z", pcl::ComparisonOps::LT, 5.5f));
//     range_cond->addComparison(cond_z2);
//         pcl::ConditionalRemoval<pcl::PointXYZ> condrem;
//     condrem.setCondition(range_cond);
//     condrem.setInputCloud(msg);
